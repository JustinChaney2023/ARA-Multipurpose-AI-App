...
export async function categorizeAndValidateWithLLM(
  ...
  return {
    form,
    validationNotes: validation.issues.map(
      i => `${i.field}: ${i.issue}${i.suggestion ? ` (${i.suggestion})` : ''}`
    ),
    rawOutput:
      categorized.rawOutput + '\n\nValidation:\n' + JSON.stringify(validation.issues, null, 2),
  };
}
...
async function categorizeText(
  ocrText: string
): Promise<{ data: Partial<MonthlyCareCoordinationForm>; rawOutput: string }> {
...
async function validateCategorizedData(
  ...
  // Clean header fields
  if (form.header) {
    form.header.recipientName = checkPlaceholder(
    ...
    form.header.recipientIdentifier = checkPlaceholder(
  ...
  return {
    ...
    narrative: {
      recipientAndVisitObservations:
        data.narrative?.recipientAndVisitObservations ||
        empty.narrative.recipientAndVisitObservations,
      healthEmotionalStatus:
        data.narrative?.healthEmotionalStatus || empty.narrative.healthEmotionalStatus,
      reviewOfServices: data.narrative?.reviewOfServices || empty.narrative.reviewOfServices,
      progressTowardGoals:
        data.narrative?.progressTowardGoals || empty.narrative.progressTowardGoals,
    ...
    signature: {
      careCoordinatorName:
        data.signature?.careCoordinatorName || empty.signature.careCoordinatorName,

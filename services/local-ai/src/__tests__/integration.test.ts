...
describe('Integration: Full OCR -> Parse Pipeline', () => {
  ...
  describe('LLM Enhancement (if available)', () => {
    it.skipIf(!ollamaAvailable)(
      'should use LLM to categorize messy text',
      async () => {
        const messyText = `
      ...
      `;
        ...
        expect(['llm-structured', 'llm-categorized', 'ocr-only']).toContain(
    ...
    it.skipIf(!ollamaAvailable)(
      'should validate and correct dates',
      async () => {
      ...
      `;
        ...
        if (result.extractionMethod === 'llm-categorized') {
          expect(result.form.narrative.additionalNotes.toLowerCase()).toMatch(

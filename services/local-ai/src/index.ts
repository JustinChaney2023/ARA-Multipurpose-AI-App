...
import {
  ...
  deleteFolder,
  addPatientToFolder,
  removePatientFromFolder,
  createSession,
  listSessionsForPatient,
  getSession,
  deleteSession,
  createSummary,
  listSummariesForSession,
  getSummary,
  getPatientWithSessions,
  createChatTurn,
  listChatTurnsForPatient,
  deleteChatTurn,
  getOrCreateUnassignedPatient,
  importLegacyHistoryItems,
    ...
    try {
      ...
      if (patientId && typeof patientId === 'number') {
        try {
          ...
          logger.info('[SUMMARIZE] RAG context retrieved', {
            patientId,
      ...
      if (patientId && typeof patientId === 'number' && !summary.isFallback) {
        try {
          ...
          const summaryRecord = createSummary(
            session.id,
            summary.summary,
            'summarizer.main',
            DEFAULT_MODEL
    ...
    try {
      ...
      if (parsedPatientId && !isNaN(parsedPatientId)) {
        try {
          ...
          ragContext = rag.context || undefined;
          logger.info('[SUMMARIZE/FILE] RAG context retrieved', {
      ...
      if (parsedPatientId && !isNaN(parsedPatientId) && !summary.isFallback) {
        try {
          ...
          summaryId = summaryRecord.id;
          logger.info('[SUMMARIZE/FILE] Persisted to DB', {
            patientId: parsedPatientId,
            sessionId,
            summaryId,
          });
...
app.post(
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      logger.info('Filling form using focused narrative Q&A', {
        requestId,
...
app.post(
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      logger.info('PDF preview complete', {
        requestId,
        size: `${(pdfBuffer.length / 1024).toFixed(2)}KB`,
      });

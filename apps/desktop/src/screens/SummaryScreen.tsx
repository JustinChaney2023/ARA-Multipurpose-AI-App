    ...
    <div className="screen" style={{ maxWidth: 680, margin: '0 auto', padding: '2rem 0' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <Btn variant="secondary" size="sm" onClick={onNew}>+ New</Btn>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
        ...
        {showRaw && (
          <div style={{ marginTop: '0.75rem' }}>
            ...
            <pre style={{
              ...
              background: 'var(--bg)', padding: '1rem', borderRadius: 8,
              border: '1px solid var(--border)', fontSize: 12.5, lineHeight: 1.65,
              margin: 0, maxHeight: '18rem', overflow: 'auto', color: 'var(--text-muted)',
            }}>
...
function renderInline(text: string): React.ReactNode[] {
  ...
  while ((match = boldPattern.exec(text)) !== null) {
    ...
    parts.push(<strong key={`b${key++}`} style={{ color: 'var(--text)', fontWeight: 600 }}>{match[1]}</strong>);
